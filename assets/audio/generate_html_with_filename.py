...
<body>
    <div class="container">
        ...
        <div class="instructions">
            <h3>📖 使用说明</h3>
            <ol>
                <li><strong>单个生成</strong>：点击任意卡片的"📋 复制（含文件名）"按钮</li>
...
for track in tracks:
    ...
    html_content += f'''
                <div class="track-card" id="track-{track['id']}">
                    ...
                    <div class="prompt-box" onclick="copyText('{full_prompt.replace("'", "\\'")}')">
                        <div class="prompt-label">📋 点击复制提示词（已含文件名）</div>
                        <div class="prompt-text">{full_prompt}</div>
                    </div>
                    <div class="track-actions">
                        <button class="btn-copy" onclick="copyText('{full_prompt.replace("'", "\\'")}'); event.stopPropagation();">
                            📋 复制
                        </button>
...
for i, track in enumerate(tracks, 1):
    full_prompt = f"[FILENAME:{track['filename']}] {track['suno_prompt']}"
    html_content += f'''
            allPrompts += `【{i}】{track['title']}\n`;
            allPrompts += `{full_prompt}\n\n`;
'''
